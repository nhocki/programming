...
// Given the suffix array for S
// We have that height[i] is the length
//   of the common prefix between the suffix `i`
//   and the suffix `i-1` in lexycographical order.
void get_the_biggest_repeated_substring(){
  ...
  for(int i=1;i<n;++i){
    if(longest < height[i]) {
        // If the common prefix is longer than the
        // best result so far, change it.
      longest = height[i]; position = i - 1;
    }
  }
  int cnt = 1;
  for(int i=position+1;i<n;++i){
      // Count how many times we see the repeated substring
    if(height[i] >= longest) cnt++;
    else break;
      // You'll need to break when it fails for the first time
      // because there may be other substrings of the same lenght
      // repeated, but we should only count the 'smallest' one.
  }
  if(longest != 0)
...
void repeated_m_times(int m){
  ...
  for(int i=0;i<=n-m;++i){
    if((t=lcp(i,i+m-1,n)) > length){
      length = t;
      position = pos[i];
      }else if(t == length) { position = min(position, pos[i]); }
    }
  ...
  //scanf("%d", &T);
    while(true){
      cin >> s;
      get_the_biggest_repeated_substring();
    }
    return 0;
  }
